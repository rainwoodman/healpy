...
The resolution of the map is defined by the *NSIDE* parameter, the :py:func:`~healpy.pixelfunc.nside2nipx` function gives the number of pixel *NPIX* of the map:
...
:py:func:`~healpy.visufunc.mollzoom` is a powerful tool for interactive inspection of a map, it provides a mollweide projection where you can click to set the center of the adjacent gnomview panel. 
...
There is an alternative way of dealing with UNSEEN pixel based on the numpy MaskedArray class, :py:func:`~healpy.fitsfunc.ma` loads a map as a masked array:
